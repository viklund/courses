...
For this we will use whole genome sequence data from two cancer cell lines, and from cell lines derived from normal tissue from the same donors. The data is part of a publicly available data set that has been used for benchmarking mutation detection algorithms. https://https://www.synapse.org/#!Synapse:syn312572/wiki/62018. We will work with the two samples HCC1143 and HCC1954, both originating from ductal carcinoma of the breast. The corresponding normal samples HCC1143_BL and HCC1954_BL are derived from blood from the same individuals.  
