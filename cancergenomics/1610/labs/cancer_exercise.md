...
In the first part of the exercise, you will detect somatic single nucleotide variants in cancer tumors using the algorithm MuTect, and annotate the detected variants with the algorithm Annovar. 
The analysis will be done on whole genome sequence data derived from breast cancer cell lines (tumor data), as well as from blood samples from the same individuals (normal data). Your analysis will be limited to a small region of chromosome 17 (17:1000000-9000000), simply because the entire genome would take days to analyze and the important thing is that you learn how to use the algorithms. 
The data is part of a publicly available breast cancer data set that has been used for benchmarking mutation detection algorithms. Two tumor/normal sample pairs with sample ids HCC1143 and HCC1954 are available. You will start with .bam files that were generated according to GATK's best practice for variant discovery (tumor and normal .bam files were generated separately). 
