...
The analysis will be done on whole genome sequence data derived from two breast cancer cell lines (the two samples can be thought of as two individuals). You are provided with sequence data from the tumor sample and from a matched normal sample that should be used to filter away the germline variants. Your analysis will be limited to a small region of chromosome 17 (17:1000000-9000000), simply because the entire genome would take days to analyze and the important thing is that you learn how to use the algorithms. 
